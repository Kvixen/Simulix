...
fmi2ValueReference vrStates[max(NUMBER_OF_STATES, 1)] = STATES;
#endif

#ifdef FMUHASRESOURCES
fmi2String convertResourcePathToLocalPath(fmi2String resourceLocationUri)
{
#ifdef WIN32
        char* localPath = (char*)calloc(MAX_PATH, sizeof(char));
        DWORD size = MAX_PATH;
        if (PathCreateFromUrlA((char*)resourceLocationUri, localPath, &size, 0) == S_OK)
        {
            return (fmi2String)localPath;
        }
        else
        {
            free(localPath);
...
static fmi2Boolean vrOutOfRange(ModelInstance *comp, const char *f, fmi2ValueReference vr, size_t end) {
    if (vr >= end) {
...
#ifdef FMUHASRESOURCES
    comp->resourceLocation = convertResourcePathToLocalPath(fmuResourceLocation);
#endif
    ...
    for (i = 0; i < nvr; i++) {
        ...
        if (value[i] == NULL) {
            ...
            if (string == NULL || strlen(string) < strlen(value[i])) {
                if (string) comp->functions->freeMemory(string);
                comp->s[vr[i]] = (fmi2String)comp->functions->allocateMemory(1 + strlen(value[i]), sizeof(char));
                if (!comp->s[vr[i]]) {
                    comp->state = modelError;
